...
static bool8 LoseExpParty(void);
...
{
    const u8 *script = GetCoordEventScriptAtPosition(&gMapHeader, COORDS_TO_GRID(position->x) - MAP_OFFSET, COORDS_TO_GRID(position->y) - MAP_OFFSET, position->elevation);
